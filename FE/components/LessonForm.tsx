          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Emoji thumbnail
                <span className="ml-1 text-xs text-gray-400 font-normal">(icon đại diện bài học trong app)</span>
              </label>
              <div className="flex gap-2 items-center">
                <span className="text-3xl w-10 text-center">{form.thumbnail_emoji || '📚'}</span>
                <input
              ...
              <div className="flex gap-1.5 mt-1.5 flex-wrap">
                {['💰','💳','🏦','🎯','📈','🛒','💡','🌱','🎁','🏆'].map(e => (
                  <button key={e} type="button" onClick={() => set('thumbnail_emoji', e)}
                    className="text-xl hover:scale-110 transition-transform">{e}</button>
                ))}
              </div>
            </div>
